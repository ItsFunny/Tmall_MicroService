...
import com.tmall.server.store.provider.service.impl.CommonMessageServiceImpl;
...
public class TmallStoreServerConfiguration implements WebMvcConfigurer
{
	private Logger logger = LoggerFactory.getLogger(TmallStoreServerConfiguration.class);
	@Autowired
	...
	@Bean
	public ICommonMessageService commonMessageService()
	{
		return new CommonMessageServiceImpl();
	}
	...
	// {
	// AbstractLoginHandler handler = new
	// EmailLoginHandler(AbstractLoginHandler.EMAIL_TYPE);
	// handler.setNextHandler(new
	...
	// registry.addInterceptor(restAuthTokenInterceptor()).addPathPatterns("/auth/**");
	// }
